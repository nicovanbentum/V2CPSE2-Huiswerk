...
int main(int argc, char *argv[]) {
  ...
  ball my_ball{sf::Vector2f{320.0, 240.0}, 30, sf::Vector2f(-5, 5)};
  walls my_walls(sf::Vector2f(5, 5), sf::Vector2f(620, 460), 10);

  square my_square(sf::Vector2f(20, 20), sf::Vector2f(100, 15));

  action actions[] = {
      action(true, [&]() { my_ball.move(sf::Vector2f(0, my_ball.speed.y)); }),
      action([&]() -> bool { return my_ball.overlaps(my_square); },
             [&]() {
               my_ball.move(sf::Vector2f(0, -1 * my_ball.speed.y));
               my_ball.speed.y *= -1;
             }),
      ...
      action(true, [&]() { my_ball.move(sf::Vector2f(my_ball.speed.x, 0)); }),
      action([&]() -> bool { return my_ball.overlaps(my_square); },
             [&]() {
               my_ball.move(sf::Vector2f(-1 * my_ball.speed.x, 0));
               my_ball.speed.x *= -1;
             }),
      action([&]() -> bool { return my_ball.overlaps(my_walls); },
             [&]() {
      ...
      action(sf::Keyboard::A,
             [&]() { my_square.move(sf::Vector2f(-5.0, 0.0)); }),
      action(sf::Keyboard::D,
             [&]() { my_square.move(sf::Vector2f(+5.0, 0.0)); }),
      action(sf::Keyboard::W,
             [&]() { my_square.move(sf::Vector2f(0.0, -5.0)); }),
